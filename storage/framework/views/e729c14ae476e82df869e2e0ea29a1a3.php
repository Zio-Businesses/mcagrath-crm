<?php if(global_setting()->system_update == 1 &&  in_array('admin', user_roles())): ?>
    ...
    <?php if($universal && config(strtolower($universal) . '.envato_item_id')): ?>
      <?php if($version[config(strtolower($universal) . '.envato_item_id')] > File::get($universal->getPath() . '/version.txt') && (config(strtolower($universal) . '.setting')::first()?->notify_update)): ?>
        <div class="col-md-12">
...
<?php endif; ?>
    ...
    <?php else: ?>
        ...
        <?php $__currentLoopData = $allModules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$module): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <?php if(config(strtolower($module) . '.envato_item_id') && $version[config(strtolower($module) . '.envato_item_id')] > File::get($module->getPath() . '/version.txt') && (config(strtolower($module) . '.setting')::first()?->notify_update)): ?>
...
<?php endif; ?>
    ...
    <?php endif; ?>
<?php endif; ?>
<?php /**PATH /home/u659716913/domains/crm.mcresi.com/public_html/resources/views/dashboard/update-message-module-dashboard.blade.php ENDPATH**/ ?>