...
<?php $component->withAttributes(['type' => 'info']); ?>
            <?php echo app('translator')->get('app.viewedOn'); ?> <?php echo e($invoice->last_viewed->timezone($settings->timezone)->translatedFormat($settings->date_format)); ?>
            ...
            <?php if(request()->ip() == $invoice->ip_address): ?>
                <strong>(<?php echo app('translator')->get('modules.invoices.sameIp'); ?>)</strong>
            <?php endif; ?>
    ...
    <div class="card-body">
        <div class="invoice-table-wrapper">
            <table width="100%" class="">
                <tr class="inv-logo-heading">
                    ...
                    <td align="right" class="font-weight-bold f-21 text-dark text-uppercase mt-4 mt-lg-0 mt-md-0">
                        <?php echo app('translator')->get('modules.lead.proposal'); ?></td>
                </tr>
                <tr class="inv-num">
                    ...
                    <td align="right">
                        <table class="inv-num-date text-dark f-13 mt-3">
                            ...
                            <tr>
                                ...
                                <td class="border-left-0"><?php echo e($invoice->valid_till->translatedFormat(company()->date_format)); ?>
            ...
            <table width="100%">
                <tr class="inv-unpaid">
                    <td class="f-14 text-dark">
                        <?php if($invoice->lead && ($invoice->lead->contact->client_name || $invoice->lead->contact->client_email || $invoice->lead->contact->mobile || $invoice->lead->contact->company_name || $invoice->lead->contact->address) && (invoice_setting()->show_client_name == 'yes' || invoice_setting()->show_client_email == 'yes' || invoice_setting()->show_client_phone == 'yes' || invoice_setting()->show_client_company_name == 'yes' || invoice_setting()->show_client_company_address == 'yes')): ?>
                        <p class="mb-0 text-left">
                            <span class="text-dark-grey text-capitalize">
                                <?php echo app('translator')->get("modules.invoices.billedTo"); ?>
                            </span><br>

                            <?php if($invoice->deal && !empty($invoice->deal->name)): ?>
                                <?php echo e($invoice->deal->name); ?><br>
                            <?php endif; ?>
                            <?php if($invoice->lead->contact && $invoice->lead->contact->client_name && invoice_setting()->show_client_name == 'yes'): ?>
                                <?php echo e($invoice->lead->contact->client_name_salutation); ?><br>
                            <?php endif; ?>
                            <?php if($invoice->lead->contact && $invoice->lead->contact->client_email && invoice_setting()->show_client_email == 'yes'): ?>
                                <?php echo e($invoice->lead->contact->client_email); ?><br>
                            <?php endif; ?>
                            <?php if($invoice->lead->contact && $invoice->lead->contact->mobile && invoice_setting()->show_client_phone == 'yes'): ?>
                                <?php echo e($invoice->lead->contact->mobile); ?><br>
                            <?php endif; ?>
                            <?php if($invoice->lead->contact && $invoice->lead->contact->company_name && invoice_setting()->show_client_company_name == 'yes'): ?>
                                <?php echo e($invoice->lead->contact->company_name); ?><br>
                            <?php endif; ?>
                            <?php if($invoice->lead->contact && $invoice->lead->contact->address && invoice_setting()->show_client_company_address == 'yes'): ?>
                                <?php echo nl2br($invoice->lead->contact->address); ?>
                    ...
                    <td align="right" class="mt-4 mt-lg-0 mt-md-0">
                        <span
                            class="unpaid <?php echo e($invoice->status == 'waiting' ? 'text-warning border-warning' : ''); ?> <?php echo e($invoice->status == 'accepted' ? 'text-success border-success' : ''); ?> rounded f-15 "><?php echo app('translator')->get('modules.proposal.'.$invoice->status); ?></span>
                    </td>
                </tr>
                <tr>
                                            ...
                                            <tr class="text-dark f-12">
                                                <td colspan="6" class="border-bottom-0">
                                                    ...
                                                    <?php if($item->proposalItemImage): ?>
                                                        <p class="mt-2">
                                                            <a href="javascript:;" class="img-lightbox" data-image-url="<?php echo e($item->proposalItemImage->file_url); ?>">
                                                                <img src="<?php echo e($item->proposalItemImage->file_url); ?>" width="80" height="80" class="img-thumbnail">
                                                            </a>
                                                        </p>
                                                    <?php endif; ?>
                                                </td>
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" tabindex="0">
                    <li>
                        <a class="dropdown-item f-14 text-dark"
                            href="<?php echo e(url()->temporarySignedRoute('front.proposal', now()->addDays(\App\Models\GlobalSetting::SIGNED_ROUTE_EXPIRY), $invoice->hash)); ?>" target="_blank">
                            <i class="fa fa-link f-w-500 mr-2 f-11"></i> <?php echo app('translator')->get('modules.proposal.publicLink'); ?>
                        </a>
                        <a class="dropdown-item f-14 text-dark"
                            href="<?php echo e(route('proposals.download', [$invoice->id])); ?>">
                            <i class="fa fa-download f-w-500 mr-2 f-11"></i> <?php echo app('translator')->get('app.download'); ?>
                        </a>
                    </li>
                    <?php if(!$invoice->signature || $invoice->status == 'waiting'): ?>
                        <li>
                            <a class="dropdown-item openRightModal"
                                href="<?php echo e(route('proposals.edit', [$invoice->id])); ?>">
                                <i class="fa fa-edit f-w-500 mr-2 f-11"></i> <?php echo app('translator')->get('app.edit'); ?>
                            </a>
                        </li>
                    <?php endif; ?>
                    <?php if(!$invoice->signature || $firstProposal->id == $invoice->id): ?>
                            <li>
                                <a class="dropdown-item delete-table-row" href="javascript:;"
                                    data-proposal-id="<?php echo e($invoice->id); ?>">
                        ...
                        $.easyAjax({
                            ...
                            success: function(response) {
                                if (response.status == "success") {
                                    window.location.href = "<?php echo e(route('proposals.index')); ?>";
                                }
                            }
                        });
                    }
...
<?php $__env->stopPush(); ?>
<?php /**PATH /home/u659716913/domains/crm.mcresi.com/public_html/resources/views/proposals/ajax/show.blade.php ENDPATH**/ ?>