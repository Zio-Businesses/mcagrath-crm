...
class Attendance extends BaseModel
{
    ...
    public static function userAttendanceByDate($startDate, $endDate, $userId)
    {
        ...
        $attendance = $attendance->filter(function ($item) use ($startDate) {
            return $item->clock_in_time->timezone(company()->timezone)->format('Y-m-d') == $startDate->timezone(company()->timezone)->format('Y-m-d');
        });
